...
void loop() {
  ...
  if (!client_found && udp.read(buffer, 64) > 0)
  {
    Serial.print("Client IP: ");
    udpAddr = udp.remoteIP();
    client_found = true;
    Serial.println(udpAddr);
  }
  ...
  if (client_found)
  {
    ...
    if(fb){
      udp.beginPacket(udpAddr, 63000);
      const uint8_t *data = (const uint8_t *)fb->buf;
      udp.write(data, fb->len);
      udp.endPacket();
    ...
    if (mpu.getEvent(&a, &g, &temp))
    {
      ...
      uint8_t data[64];
      serializedData.getBytes(data, sizeof(data));

      udp.beginPacket(udpAddr, 63000);
      udp.write(data, serializedData.length() + 1);
      udp.endPacket();
    }
  }
}
